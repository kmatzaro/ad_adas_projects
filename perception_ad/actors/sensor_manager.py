...
class SensorManager:
    def __init__(self, world, display_manager, perception_module, config: Dict):
        ...
        # Latest raw LiDAR point cloud — stored for depth fusion
    ...
    def restart_sensor(self, sensor, sensor_type, sensor_id: str, attach_to):
        """
        ...
        try:
            ...
            new_sensor = self.init_sensor(sensor_type, attach_to, sensor_id)
            print(f"{sensor_type} restart successful")
            return new_sensor

        except Exception as e:
            print(f"{sensor_type} restart failed: {e}")
            return None
    ...
    def camera_callback(self, image, camera_id: str):
        ...
        try:
            ...
            if camera_id == 'front_camera':
                processed_frame = self.perception_module.combined_camera_process(
                    frame, camera_id, timestamp, self.latest_lidar_points
                )
    ...
    def lidar_callback(self, image):
        ...
        try:
            ...
            # Store raw points for LiDAR-camera depth fusion
            self.latest_lidar_points = points.copy()

            # Rasterize to 2-D BEV image for display
